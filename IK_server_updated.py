...
class KR210Kinematics:
    ...
    def cleanPickles(self):
        ...
        os.remove("kr210_t0_ee.p")
        os.remove("kr210_rot_ee.p")
        os.remove("kr210_r0_3.p")

    def fastcompute(self, px, py, pz, roll, pitch, yaw):
        ...
        # look for the pickle file containing the T0_EE transformation matrix
        if not os.path.exists("kr210_t0_ee.p") or not os.path.exists("kr210_r0_3.p"):
            ...
            # write the results of computation to pickle file for future use
            pickle.dump( self.T0_EE, open("kr210_t0_ee.p", "wb"))

            # we also need to store the result of this computation for future use
            self.R0_3 = self.T0_1[0:3, 0:3] * self.T1_2[0:3, 0:3] * self.T2_3[0:3, 0:3]

            pickle.dump(self.R0_3, open("kr210_r0_3.p", "wb"))

        else:

            # load the pre-computed matrixces from the pickle files
            self.T0_EE = pickle.load(open("kr210_t0_ee.p", "rb"))

            self.R0_3 = pickle.load(open("kr210_r0_3.p", "rb"))

        # look for the pickle file containing the ROT_EE transformation matrix
        if not os.path.exists("kr210_rot_ee.p"):
            ...
            # write the results of computation to pickle file for future use
            pickle.dump( self.ROT_EE, open("kr210_rot_ee.p", "wb"))

        else:

            # load the pre-computed matrix from the pickle file
            self.ROT_EE = pickle.load(open("kr210_rot_ee.p", "rb"))

        
        # At this point, we can start substituting in values to solve for joint angles
        ...
        sidea_squared = 2.253001
        sideb_squared = self.side_b * self.side_b
        sidec_squared = 1.5625
        ...
        # the idea to use transpose() instead of inv("LU") was suggested in conversation with my course mentor.
        self.R3_6 = self.R0_3.transpose() * self.ROT_EE

        # the idea to compute theta5 before computing theta4 and theta6 was based on a hint I received
        # as part of the review of my first submission for this project
...
def handle_calculate_IK(req):
    ...
    if len(req.poses) < 1:
	    ...
	    # roll, pitch, yaw = end-effector orientation
            ...
            kr210.fastcompute(px, py, pz, roll, pitch, yaw)
